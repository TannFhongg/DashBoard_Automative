...
static char  current_gear = 'D';
static float trip_km      = 0.0f;


// KHỞI TẠO ADC
// Sử dụng độ phân giải 12-bit (0-4095)
// Attenuation 12dB → đo được điện áp 0-3.3V

static void adc_init(void)
{
    adc1_config_width(ADC_WIDTH_BIT_12);
    adc1_config_channel_atten(ADC_RPM_CHANNEL,   ADC_ATTEN_DB_12);
    adc1_config_channel_atten(ADC_SPEED_CHANNEL, ADC_ATTEN_DB_12);
}


// ĐỌC VÀ CHUYỂN ĐỔI ADC -> GIÁ TRỊ THỰC
// lấy trung bình 8 mẫu để giảm nhiễu ADC
...
// Thuật toán debounce: Chỉ ghi nhận thay đổi
...
{
    buttons[0] = (ButtonDebounce){ BTN_P, 'P', 1, 1, 0 };
    buttons[1] = (ButtonDebounce){ BTN_R, 'R', 1, 1, 0 };
    buttons[2] = (ButtonDebounce){ BTN_N, 'N', 1, 1, 0 };
    buttons[3] = (ButtonDebounce){ BTN_D, 'D', 1, 1, 0 };
}
